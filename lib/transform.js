...
function getVal(before, remainder, data) {
  ...
  try {
    ...
    // no node found, put markers to delete from output
    if (typeof result === 'undefined') result = '';
    // null is a valid usecase
    else if (typeof result === null) result = null;
    else if (typeof result !== 'object' && isNumeric(result) && (before !== '' || after !== '')) {
      result = result.toString();
    }
    // empty string is also a valid usecase
    // both null and empty string would lead to null value for the field
    ...
    try {
      ...
      // null is a valid usecase
        ...
        result = result.toString();
        // appending before and after string to integer 
        result = before + result + after;
      }
      // empty string is also a valid usecase
      // both null and empty string would lead to null value for the field
  ...
  var outputObj = traverse(templateObj).map(function(node) {
    ...
    if (typeof node === 'function') {
      ...
      if (node.indexOf('{<') !== -1) {
        ...
        // console.log("The values are :%s", node);
      }
      ...
      if (node[0] === '=' && node[1] === '>') {
          ...
          args.forEach(function(arg) {
            ...
            if (arg[0] === '"' || arg[0] === "'") {
              ...
              try {
                ...
                // console.log("The argument value :%o", arg);
              }
              catch(err) {}
            }
            argArr.push(arg);
        ...
        try {
          ...
          // console.log("The arguments :%o", argArr);
